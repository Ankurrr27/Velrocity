          ...
          return (
            <button
              ...
              className={`
                ...
                ${isSelected 
                  ? 'z-10 scale-[1.05] border border-[rgba(var(--primary-rgb),0.3)] bg-white dark:bg-zinc-800 shadow-[0_15px_30px_-10px_rgba(var(--primary-rgb),0.3)]' 
                  : 'border border-transparent bg-white/5 hover:bg-white/10 dark:bg-white/[0.03] dark:hover:bg-white/[0.06]'
                }
              `}
            >
              <span className={`mb-0.5 sm:mb-1.5 text-[7px] sm:text-[8px] font-black uppercase tracking-[0.15em] ${isSelected ? 'text-[rgb(var(--primary))]' : 'text-zinc-500'}`}>
              ...
              {hasHabit && !isSelected && (
                <div className="absolute bottom-2 h-1 w-1 rounded-full bg-[rgba(var(--primary-rgb),0.4)]" />
              )}
              {isSelected && (
                <motion.div 
                  layoutId="calendar-indicator-elite"
                  className="absolute bottom-0 left-0 right-0 h-1 rounded-full bg-[rgb(var(--primary))]"
                />
              )}
            </button>
          );
